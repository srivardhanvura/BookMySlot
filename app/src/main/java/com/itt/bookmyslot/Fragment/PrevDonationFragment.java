...
public class PrevDonationFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_prev_donate, container, false);

        final EditText et1 = view.findViewById(R.id.donRegNo);
        final EditText et2 = view.findViewById(R.id.donPass);
        final TextView txtView = view.findViewById(R.id.donate_text);
        Button btn = view.findViewById(R.id.submit_prev_don);
        res = "Your previous bookings were on:\n";

        btn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(final View view) {
                res = "Your previous donations were:\n";
                            ...
                            for (DataSnapshot data2 : snapshot.getChildren()) {
                                if (reg.equals(data2.getKey())) {
                                    ...
                                    if (!actualPass.equals(pass)) {
                                        ...
                                        DatabaseReference db2 = FirebaseDatabase.getInstance().getReference().child("prev_donations");
                                            ...
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                for (DataSnapshot data : snapshot.getChildren()) {
                                                    for (DataSnapshot data2 : data.getChildren()) {
                                                        ...
                                                        res = res.concat("Rs. "+resArr[0] + " on " + resArr[1] + "\n");
                                                        count[0] = true;
                                                    }
                                                    txtView.setText(res);
                                                }
                                                if (!count[0]) {
                                                    txtView.setText("No donations found\n");
                                                    found[0] = false;
                                                }
                                            }
